...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T2, Tensor<__half, 1> T4, Tensor<__half, 2> T20, Tensor<float, 2> T27, Tensor<float, 2> T29) {
  alignas(16) extern __shared__ char array[];
  void* shared_mem = array;
  NVFUSER_DEFINE_MAGIC_ZERO
  int i217;
  i217 = ((nvfuser_index_t)blockIdx.x) * T0.size[1];
  int i74;
  i74 = ceilDiv((ceilDiv(T0.size[1], 8)), 10);
  bool b852;
  b852 = ((nvfuser_index_t)threadIdx.x) < i74;
  bool b196;
  b196 = ((nvfuser_index_t)threadIdx.x) == 0;
  double d16;
  d16 = (double)(T0.size[1]);
  double d17;
  d17 = 1.00000000000000000e+00 * d16;
  double d25;
  d25 = reciprocal(d17);
  double d49;
  d49 = (double)(T0.size[1]);
  double d50;
  d50 = 1.00000000000000000e+00 * d49;
  float T1[((10 * 1) * 8)];
  #pragma unroll
  for(nvfuser_index_t i160 = 0; i160 < 10; ++i160) {
    int i220;
    i220 = 8 * (((nvfuser_index_t)threadIdx.x) + (i74 * (i160 + nvfuser_zero)));
    int i221;
    i221 = i217 + i220;
    int i238;
    i238 = 8 * i160;
    bool b855;
    b855 = b852 && ((7 + i220) < T0.size[1]);
    if (b855) {
      Array<__half, 8, 8> T23;
      T23.set(0);
      loadGlobalToLocal<__half, 8, false>(&T23[0],  &T0[i221]);
      #pragma unroll
      for(nvfuser_index_t i162 = 0; i162 < 8; ++i162) {
        T1[(i238 + i162)]
           = __half2float(T23[i162]);
      }
    } else {
      Array<__half, 8, 8> T23;
      if (b852) {
  ...
  float T21[1];
  T21[0] = 0.00000000000000000e+00;
  float T31[1];
  if (b852) {
    T31[0] = 0.00000000000000000e+00;
  }
  #pragma unroll
  for(nvfuser_index_t i163 = 0; i163 < 10; ++i163) {
    int i301;
    i301 = 8 * i163;
    bool b985;
    b985 = b852 && ((7 + (8 * (((nvfuser_index_t)threadIdx.x) + (i74 * (i163 + nvfuser_zero))))) < T0.size[1]);
    if (b985) {
      #pragma unroll
      for(nvfuser_index_t i165 = 0; i165 < 8; ++i165) {
        T31[0]
          = T31[0]
          + T1[(i301 + i165)];
      }
  ...
  T28[0]
     = T9[0];
  if (b196) {
    T27[((nvfuser_index_t)blockIdx.x)]
       = T28[0];
  }
  // Alias Allocation - register
  auto& T10 = T1;
  #pragma unroll
  for(nvfuser_index_t i167 = 0; i167 < 10; ++i167) {
    int i385;
    i385 = 8 * i167;
    if (b852) {
      #pragma unroll
      for(nvfuser_index_t i169 = 0; i169 < 8; ++i169) {
        int i386;
        i386 = i385 + i169;
        T10[i386]
          = T1[i386]
  ...
  #pragma unroll
  for(nvfuser_index_t i170 = 0; i170 < 10; ++i170) {
    int i468;
    i468 = 8 * i170;
    bool b1116;
    b1116 = b852 && ((7 + (8 * (((nvfuser_index_t)threadIdx.x) + (i74 * (i170 + nvfuser_zero))))) < T0.size[1]);
    if (b1116) {
      #pragma unroll
      for(nvfuser_index_t i172 = 0; i172 < 8; ++i172) {
        int i469;
        i469 = i468 + i172;
        float T22[1];
        T22[0]
  ...
  T14[0]
     = rsqrtf(T13[0]);
  #pragma unroll
  for(nvfuser_index_t i184 = 0; i184 < 10; ++i184) {
    int i606;
    i606 = 8 * (((nvfuser_index_t)threadIdx.x) + (i74 * (i184 + nvfuser_zero)));
    int i638;
    i638 = 8 * i184;
    int i677;
    i677 = i217 + i606;
    bool b1307;
    b1307 = b852 && ((7 + i606) < T0.size[1]);
    if (b1307) {
      Array<__half, 8, 8> T25;
      T25.set(0);
      loadGlobalToLocal<__half, 8, false>(&T25[0],  &T4[i606]);
      Array<__half, 8, 8> T24;
      T24.set(0);
      loadGlobalToLocal<__half, 8, false>(&T24[0],  &T2[i606]);
      // Alias Allocation - register
      auto& T26 = T24;
      #pragma unroll
      for(nvfuser_index_t i183 = 0; i183 < 8; ++i183) {
        float T3[1];
        T3[0]
           = __half2float(T24[i183]);
        float T16[1];
        T16[0]
           = T3[0];
        float T5[1];
        T5[0]
           = __half2float(T25[i183]);
        float T18[1];
        T18[0]
           = T5[0];
        float T15[1];
      ...
      loadLocalToGlobal<__half, 8, false>( &T20[i677], &T26[0]);
    } else {
      Array<__half, 8, 8> T25;
      if (b852) {
        T25.set(0);

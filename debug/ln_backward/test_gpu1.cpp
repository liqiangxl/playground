...
TEST_F(NVFuserTest, FusionMagicSchedulerLayerNormBackward_CUDA) {
  ...
  std::vector<int64_t> shape{2048,10240};
  std::vector<int64_t> norm_shape{10240};
  ...
  auto grad_out = makeSymbolicTensor(shape.size(), dtype);
  auto input = makeSymbolicTensor(shape.size(), dtype);
  auto mean = makeConcreteTensor(outer_shape, DataType::Float);
  auto rstd = makeConcreteTensor(outer_shape, DataType::Float);
  auto weight = makeSymbolicTensor(norm_shape.size(), dtype);
  auto bias = makeSymbolicTensor(norm_shape.size(), dtype);
  ...
  if (dtype == DataType::Half) {
    layer_norm_results.grad_input  = castOp(dtype, layer_norm_results.grad_input);
    layer_norm_results.grad_bias   = castOp(dtype, layer_norm_results.grad_bias);
    layer_norm_results.grad_weight = castOp(dtype, layer_norm_results.grad_weight);
  }
  ...
  auto fp32_options =
      at::TensorOptions().dtype(at::kFloat).device(at::kCUDA, 0);
  at::Tensor aten_grad_out = at::randn(shape, maybe_fp16_options);
  at::Tensor aten_input = at::randn(shape, maybe_fp16_options);
...
TEST_F(NVFuserTest, FusionMagicSchedulerLayerNormalization_CUDA) {
  ...
  auto result_output = dtype == DataType::Half ? castOp(dtype, result.output) : result.output;
  ...
  auto reduction_params = getPersistentHeuristics(&fusion, {aten_input, aten_weight, aten_bias});
  TORCH_CHECK(reduction_params, "Reduction schedule was not generated!");

  FusionExecutorCache fec(std::move(fusion_ptr));
  auto cg_outputs = fec.runFusionWithInputs({aten_input, aten_weight, aten_bias});
...
TEST_F(NVFuserTest, FusionMagicSchedulerBatchNormalizationChanelLast_CUDA) {
  // 10x100x8x8, translated: 33 GB/s, welford: 28 GB/s
  // 20x100x35x45, translation not triggered.
  ...
  const float kEps = 1e-5;
  const bool kTraining = true;
  const bool kChannelLast = true;
  const bool kCUDNN = true;
  constexpr int n = 20;
  constexpr int c = 100;
  constexpr int h = 18;
  constexpr int w = 18;
  ...
  auto result = batch_norm(
      input, weight, bias, running_mean, running_var, kTraining, momentum, eps, kChannelLast);
  ...
  auto options = at::TensorOptions().dtype(at::kFloat).device(at::kCUDA, 0);
  auto at_input = kChannelLast ? at::randn({n,h,w,c}, options) : at::randn(input_shape, options);             
  auto at_weight = at::ones({c}, options);
  auto at_bias = at::zeros({c}, options);
  auto at_run_mean = at::zeros({c}, options);
  auto at_run_var = at::ones({c}, options);
  ...
  auto aten_outputs = at::batch_norm(
      kChannelLast ? at::randn({n,c,h,w}, options).contiguous(c10::MemoryFormat::ChannelsLast) : at::randn({n,c,h,w}, options),
      c10::optional<at::Tensor>(at_weight),
      c10::optional<at::Tensor>(at_bias),
      c10::optional<at::Tensor>(at_run_mean),
      c10::optional<at::Tensor>(at_run_var),

...
// a small case to make sure welford op is not translated in outer normalilzaiton
TEST_F(NVFuserTest, FusionMagicSchedulerGridPersistentWelfordOuterNorm_CUDA) {
  // result on A100
  // 25*7*512  --> segmented (N), Welford converted (Y), 163 GB/s
  // 25*7*512  --> segmented (N), Not Allow conversion,  90 GB/s
  //=========================== step-0, parameters for this test
  int64_t N  = 25;
  int64_t HW = 7;
  int64_t C  = 512;
  DataType dtype = DataType::Half;
  //=========================== step-1, define fusion (what to compute)
  std::unique_ptr<Fusion> fusion_ptr = std::make_unique<Fusion>();
  Fusion& fusion = *fusion_ptr.get();
  FusionGuard fg(&fusion);
  ...
  at::manual_seed(0);
  const std::vector<int64_t> input_shape{N, HW, HW, C};
  at::Tensor t0 = at::randn(input_shape, options_half);

  //=========================== step-3, get heuristics (how to compute)
  auto reduction_params = getPersistentHeuristics(&fusion, {t0});
  ...
  //=========================== step-5, validate result and check WelfordOp exists or not
  auto exprs = fusion.exprs();
  std::vector<WelfordOp*> welfords(
      ir_utils::filterByType<WelfordOp>(exprs).begin(),
      ir_utils::filterByType<WelfordOp>(exprs).end());
  std::cout << "welfords.size()= " << welfords.size() << std::endl;
  TORCH_CHECK(welfords.size() > 0, "No Welfordop detected! check if it was converted!");

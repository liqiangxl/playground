...
std::unique_ptr<SegmentedFusion> SegmentedFusion::fromCompleteFusion(
  ...
  bool welford_converted = SegmentCandidateFinder::TranslateWelfordInFusion(fusion,runtime_inputs);
  std::cout << "welford_converted= " << welford_converted << std::endl;
...
class TranslateApplicableWelford {
 public:
  ...
  static bool run(
    ...
    std::cout << "welford has been translated= " << translate_welford.translated_any_welford_ << std::endl;
 ...
 private:
  ...
  //! reuse_x_mean_sub may cause large persistent buffer
  //! set default to false to save registers for reuse of x.
  void translateSingleWelford(WelfordOp* welford, bool reuse_x_mean_sub = false);
...
bool TranslateApplicableWelford::wouldTranslateToPersistent(
  ...
  if (group != nullptr) {
    ...
    std::cout << "segmented fusion: isValidPersistentFusion= " << isValidPersistentFusion(test_copy.get(), runtime_info) << std::endl;
  ...
  //  translate and test.
  std::cout << "un-segmented fusion: isValidPersistentFusion= " << isValidPersistentFusion(test_copy.get(), runtime_info) << std::endl;
  return isValidPersistentFusion(test_copy.get(), runtime_info);
}

void TranslateApplicableWelford::translateSingleWelford(WelfordOp* welford, bool reuse_x_mean_sub) {
  ...
  // x_mean_sub may already exist. Reuse it if allow_reuse and found.
  TensorView* x_mean_sub = nullptr;
  if (reuse_x_mean_sub && x_avg_bcast != nullptr) {
...
void SegmentCandidateFinder::findSegments() {
  ...
  if (options_.run_translate_welford &&
    ...
    std::cout << "try to TranslateApplicableWelford" << std::endl;
    TranslateApplicableWelford::run(segmented_fusion_.get(), runtime_inputs_);
  }else{
    std::cout << "no welford_ops" << std::endl;
  }

...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T1, Tensor<__half, 1> T2, Tensor<__half, 2> T20, Tensor<float, 2> T29, Tensor<float, 2> T31) {
  ...
  T35[0] = 0.0f;
  T33[0] = 0.0f;
  int active_warps = blockDim.x / 32;
  if(lane < active_warps){
    T35[0] = shared_mem_avg[lane];
    T33[0] = shared_mem_var[lane];   
  }
  warp_chan_upd_dynamic(T35[0], T33[0], T34[0], active_warps);
  T9[0]  = T35[0];
  T11[0] = T33[0];
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  float T30[1];

...
{
  int elements_per_row = T0.size[1];
  int nrow = T0.size[0];
  ...
  float *shared_mem_avg = shared_mem_var + nwarps;
  //NVFUSER_DEFINE_MAGIC_ZERO

  float d24 = reciprocal((float)(T0.size[1]));

  // weights & bias
  Array<__half, ((10 * 1) * 8), 8> weights;
  Array<__half, ((10 * 1) * 8), 8> bias;
  #pragma unroll
  for(nvfuser_index_t i172 = 0; i172 < 10; ++i172) {
    int i246 = 8 * (threadIdx.x + blockDim.x * i172);
    loadGlobalToLocal<__half, 8, false>(&weights[(8 * i172)],  &T1[i246]);
    loadGlobalToLocal<__half, 8, false>(&bias[(8 * i172)],  &T2[i246]);
  }
  ...
  for(int row = blockIdx.x; row < nrow; row += gridDim.x)
  {
    ...
    float T35  = 0.00000000000000000e+00f;
    float T33  = 0.00000000000000000e+00f;
    float T34  = 0.0f;
    ...
    for(nvfuser_index_t i175 = 0; i175 < 10; ++i175) {
      ...
      for(nvfuser_index_t i177 = 0; i177 < 8; ++i177) {
          T35 += __half2float(T25[(i279 + i177)]);
      }
    }
    // warp reduction
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
      T35 += __shfl_xor_sync(0xffffffff, T35, offset);
    }
    T34 = 80*32;
    T35 /= T34;
    ...
    for(nvfuser_index_t i175 = 0; i175 < 10; ++i175) {
      ...
      #pragma unroll
      for(nvfuser_index_t i177 = 0; i177 < 8; ++i177) {
        float T3 = __half2float(T25[(i279 + i177)]);
        T33 += (T3 - T35)*(T3 - T35);
      }
    }
    // warp reduction
    for (int offset = 16; offset > 0; offset >>= 1) {
      T33 += __shfl_xor_sync(0xffffffff, T33, offset);
    }
    // ==================== block reduction welford =====================================================
    if(lane==0){
      shared_mem_avg[wid] = T35;
      shared_mem_var[wid] = T33;
    }
    block_sync::sync();
    T35 = 0.0f;
    T33 = 0.0f;
    ...
    if(lane < nwarps){
      T35 = shared_mem_avg[lane];
      T33 = shared_mem_var[lane];   
    }
    warp_chan_upd_dynamic(T35, T33, T34, nwarps);
    T9  = T35;
    T11 = T33;
    //////////////////////////////////
    float T14 = rsqrtf(T11*d24 + 9.99999974737875164e-06F);
    if (threadIdx.x==0) {
      T29[row] = T9;
      T31[row] = T14;
    }      
    // calc output
    Array<__half, ((10 * 1) * 8), 8> output;
    #pragma unroll
    for(nvfuser_index_t i190 = 0; i190 < 10; ++i190) {
      int i475 = 8 * (threadIdx.x + blockDim.x * i190);
      int i513 = 8 * i190;
      int i571 = i243 + i475;
      {
        #pragma unroll
        for(nvfuser_index_t i189 = 0; i189 < 8; ++i189) {
          float T10 = __half2float(T25[(i513 + i189)]) - T9;
          float T16 = __half2float(weights[(i513 + i189)]);
          float T18 = __half2float(bias[(i513 + i189)]);
          float T19 = T10 * T14 * T16 + T18;
          output[(i513 + i189)] = __float2half(T19);
        }
        loadLocalToGlobal<__half, 8, false>( &T20[i571], &output[i513]);
      }
    }
  }
}

...
/*  blockWelford<true, false, false>(
  ...
  for (int offset = 16; offset > 0; offset >>= 1) {
    float avg_peer = __shfl_down_sync(0xffffffff, T35[0], offset);
    float var_peer = __shfl_down_sync(0xffffffff, T33[0], offset);
    int   N_peer   = __shfl_down_sync(0xffffffff, T34[0], offset);
  ...
  if(blockDim.x > 32){
    ...
    __syncthreads();
    // s-2, first several threads in first warp to another reduciton
    if(wid==0){
    ...
    // only one warp, each lane get data from lane-0
    T9[0]  = __shfl_sync(0xffffffff, T35[0], 0);
    T11[0] = __shfl_sync(0xffffffff, T33[0], 0);
  }
   /////////////////////////////////////////////

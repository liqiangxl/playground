...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T1, Tensor<__half, 1> T2, Tensor<__half, 2> T20, Tensor<float, 2> T29, Tensor<float, 2> T31) {
  ...
  int i243;
  i243 = ((nvfuser_index_t)blockIdx.x) * T0.size[1];
  int i84;
  i84 = ceilDiv((ceilDiv(T0.size[1], 8)), 10);
  bool b197;
  b197 = ((nvfuser_index_t)threadIdx.x) == 0;
  double d15;
  d15 = (double)(T0.size[1]);
  double d16;
  d16 = 1.00000000000000000e+00 * d15;
  double d24;
  d24 = reciprocal(d16);
  Array<__half, ((10 * 1) * 8), 8> T25;
  #pragma unroll
  for(nvfuser_index_t i172 = 0; i172 < 10; ++i172) {
    T25.set(0);
  }
  ...
  for(nvfuser_index_t i172 = 0; i172 < 10; ++i172) {
    int i246;
    i246 = 8 * (((nvfuser_index_t)threadIdx.x) + (i84 * (i172 + nvfuser_zero)));
    if (((7 + i246) < T0.size[1])) {
      loadGlobalToLocal<__half, 8, false>(&T25[(8 * i172)],  &T0[(i243 + i246)]);
    }
  }
  NVFUSER_UPDATE_MAGIC_ZERO
  float T6[1];
  T6[0] = 0.00000000000000000e+00;
  float T7[1];
  T7[0] = 0.00000000000000000e+00;
  int T8[1];
  T8[0] = 0;
  float T35[1];
  T35[0] = 0.00000000000000000e+00;
  float T33[1];
  T33[0] = 0.00000000000000000e+00;
  int T34[1];
  T34[0] = 0;
  #pragma unroll
  for(nvfuser_index_t i175 = 0; i175 < 10; ++i175) {
    int i279;
    i279 = 8 * i175;
    bool b861;
    b861 = (7 + (8 * (((nvfuser_index_t)threadIdx.x) + (i84 * (i175 + nvfuser_zero))))) < T0.size[1];
    if (b861) {
      #pragma unroll
      for(nvfuser_index_t i177 = 0; i177 < 8; ++i177) {
        float T3[1];
        T3[0]
           = __half2float(T25[(i279 + i177)]);
        welfordCombine (
          T35[0],
          T33[0],
          T34[0],
...
/*  blockWelford<true, false, false>(
  ...
  broadcast::blockBroadcast<true, false, false>(
    ...
    */
  /////////////////////////////////////////////
  // intra-warp reduction to lane-0 
  float T9[1]; // mean
  float T11[1]; // var
/*
  warp::warpReduceTIDX<false>(
    ...
    threadIdx,
    blockDim,
    static_cast<float*>(shared_mem_avg),
    true,
  ...
  warp::warpReduceTIDX<false>(
    ...
    threadIdx,
    blockDim,
    static_cast<float*>(shared_mem_var),
    true,
    float(0.00000000000000000e+00));
*/

  // intra-warp butterfly reduction, all threads have the right result
  for (int offset = 16; offset > 0; offset >>= 1) {
    float avg_peer = __shfl_xor_sync(0xffffffff, T35[0], offset);
    float var_peer = __shfl_xor_sync(0xffffffff, T33[0], offset);
    //int   N_peer   = __shfl_xor_sync(0xffffffff, T34[0], offset);
    welfordCombine(T35[0], T33[0], T34[0], avg_peer, var_peer, T34[0]);
  }
  #if 10
  if(blockDim.x > 32){
    // inter-warp reduction
    // s-1, first thread of each warp write warp reduciton result to shared mem
    int lane = threadIdx.x % 32;
    int wid  = threadIdx.x / 32;
    if(lane==0){
      shared_mem_avg[wid] = T35[0];
      shared_mem_var[wid] = T33[0];
      shared_mem_n[wid]   = T34[0];
    }
    block_sync::sync();
    // s-2, first several threads in first warp do another reduciton
    if(wid==0){
      bool activeThread = threadIdx.x < blockDim.x / 32;
      T35[0] = activeThread ? shared_mem_avg[lane] : 0.0f;
      T33[0] = activeThread ? shared_mem_var[lane] : 0.0f;
      T34[0] = activeThread ? shared_mem_n[lane] : 0;
      for (int offset = 16; offset > 0; offset >>= 1) {
        float avg_peer = __shfl_xor_sync(0xffffffff, T35[0], offset);
        float var_peer = __shfl_xor_sync(0xffffffff, T33[0], offset);
        int   N_peer   = __shfl_xor_sync(0xffffffff, T34[0], offset);
        welfordCombine(T35[0], T33[0], T34[0], avg_peer, var_peer, N_peer);
      }
      if(lane==0){
        shared_mem_avg[0] = T35[0];
        shared_mem_var[0] = T33[0];
      }    
    }
    block_sync::sync();
    // s-3, broadcast to all threads
    T9[0]  = shared_mem_avg[0];
    T11[0] = shared_mem_var[0];
  }
  else
  #endif
  {
  ...
  T30[0]
     = T9[0];
  if (b197) {
    T29[((nvfuser_index_t)blockIdx.x)]
       = T30[0];
  }
  float T12[1];
  T12[0]
  ...
  T14[0]
     = rsqrtf(T13[0]);
  #pragma unroll
  for(nvfuser_index_t i190 = 0; i190 < 10; ++i190) {
    int i475;
    i475 = 8 * (((nvfuser_index_t)threadIdx.x) + (i84 * (i190 + nvfuser_zero)));
    int i513;
    i513 = 8 * i190;
    int i571;
    i571 = i243 + i475;
    bool b1072;
    b1072 = (7 + i475) < T0.size[1];
    if (b1072) {
      Array<__half, 8, 8> T27;
      T27.set(0);
      loadGlobalToLocal<__half, 8, false>(&T27[0],  &T2[i475]);
      Array<__half, 8, 8> T26;
      T26.set(0);
      loadGlobalToLocal<__half, 8, false>(&T26[0],  &T1[i475]);
      // Alias Allocation - register
      auto& T28 = T26;
      #pragma unroll
      for(nvfuser_index_t i189 = 0; i189 < 8; ++i189) {
        float T21[1];
        T21[0]
           = __half2float(T25[(i513 + i189)]);
        float T10[1];
        T10[0]
          = T21[0]
          - T9[0];
        float T4[1];
        T4[0]
           = __half2float(T26[i189]);
        float T16[1];
        T16[0]
           = T4[0];
        float T5[1];
        T5[0]
           = __half2float(T27[i189]);
        float T18[1];
        T18[0]
           = T5[0];
        float T15[1];
      ...
      loadLocalToGlobal<__half, 8, false>( &T20[i571], &T28[0]);
    } else {
      Array<__half, 8, 8> T27;
      T27.set(0);
      if (b1072) {
  ...
  T32[0]
     = T14[0];
  if (b197) {
    T31[((nvfuser_index_t)blockIdx.x)]
       = T32[0];
  }
}
}

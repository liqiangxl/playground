...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T1, Tensor<__half, 1> T2, Tensor<__half, 2> T20, Tensor<float, 2> T29, Tensor<float, 2> T31) 
{
  ...
  for(nvfuser_index_t i172 = 0; i172 < 10; ++i172) {
    int i246 = 8 * (((nvfuser_index_t)threadIdx.x) + (i84 * (i172 + nvfuser_zero)));
    loadGlobalToLocal<__half, 8, false>(&weights[(8 * i172)],  &T1[i246]);
    loadGlobalToLocal<__half, 8, false>(&bias[(8 * i172)],  &T2[i246]);
  }
  
  // =========== grid loop =====================
  for(int row = blockIdx.x; row < T0.size[0]; row += gridDim.x)
  {
      int i243 = row * T0.size[1];
      // load input
      ...
      for(nvfuser_index_t i190 = 0; i190 < 10; ++i190) {
        ...
        if (b1072) {
          ...
          for(nvfuser_index_t i189 = 0; i189 < 8; ++i189) {
            ...
            float T4[1];
            T4[0]
              = __half2float(weights[(i513 + i189)]);
            float T16[1];
            T16[0]
              = T4[0];
            float T5[1];
            T5[0]
              = __half2float(bias[(i513 + i189)]);
            float T18[1];
            T18[0]
              = T5[0];
            float T15[1];

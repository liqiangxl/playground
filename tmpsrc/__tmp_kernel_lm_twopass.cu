...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T1, Tensor<__half, 1> T2, Tensor<__half, 2> T20, Tensor<float, 2> T29, Tensor<float, 2> T31) {
  ...
  float d24 = reciprocal((float)(T0.size[1]));
  ...
  // each thread sum 80 elements, then warp reduction of 32 threads
  // =========================== thread-warp-block reduction pass-1 =========================================  
  // thread reduction
  #pragma unroll
  for(nvfuser_index_t i175 = 0; i175 < 10; ++i175) {
    int i279 = 8 * i175;
    for(nvfuser_index_t i177 = 0; i177 < 8; ++i177) {
        T35[0] += __half2float(T25[(i279 + i177)]);
    }
  }
  // warp reduction
  #pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    T35[0] += __shfl_xor_sync(0xffffffff, T35[0], offset);
  }
  // block reduction
  ...
  block_sync::sync();
  T35[0] = lane < nwarps ? shared_mem_avg[lane] : 0.0f;
  //Assume at least leftmost is valid and init: step = next_pow2(num_active) / 2 (might get NaN otherwise)
  int highest_bit_set = (8 * sizeof(nwarps)) - __clz(nwarps - 1);  
  #pragma unroll
  for( int step = (1 << (highest_bit_set - 1)); step > 0; step /= 2 ) {
    T35[0] += warp_shuffle_down(T35[0], step);
  }  
  T35[0] = __shfl_sync(uint32_t(-1), T35[0], 0) * d24; // Intra-warp broadcast (only lane 0 has valid stats).
  T9[0]  = T35[0] ;

  // =========================== thread-warp-block reduction pass-2 =========================================
  #pragma unroll
  for(nvfuser_index_t i175 = 0; i175 < 10; ++i175) {
    int i279 = 8 * i175;
    #pragma unroll
    for(nvfuser_index_t i177 = 0; i177 < 8; ++i177) {
      ...
      T33[0] += (T3[0] - T35[0])*(T3[0] - T35[0]);
    }
  }
  // warp reduction
  for (int offset = 16; offset > 0; offset >>= 1) {
    T33[0] += __shfl_xor_sync(0xffffffff, T33[0], offset);
  }
    // block reduction
  if(lane==0){
  ...
  block_sync::sync();
  T33[0] = lane < nwarps ? shared_mem_var[lane] : 0.0f; 
  #pragma unroll
  for( int step = (1 << (highest_bit_set - 1)); step > 0; step /= 2 ) {
    T33[0] += warp_shuffle_down(T33[0], step);
  }  
  T33[0] = __shfl_sync(uint32_t(-1), T33[0], 0); // Intra-warp broadcast (only lane 0 has valid stats).
  T11[0] = T33[0];
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  float T30[1];
  T30[0]
